...
1. Read `routax-wiki/product/roadmap.md` to recall the current 7-phase plan (authoritative living roadmap). Use `routax-wiki/raw/via-roadmap.md` only as historical source context.
...
3. Read `routax-wiki/log.md` (last 20 entries) to see recent activity.
...
**Blocking decisions** (any open questions from the wiki that must be resolved before proceeding — check `routax-wiki/product/decisions/` and pages with `status: contested`)
