...
describe("RoutingProfileSchema", () => {
  it("round-trips a valid profile with preferSmoothSurfaces", () => {
    const profile = {
      avoidTraffic: 0.5,
  ...
  it("defaults preferSmoothSurfaces to 0 when absent", () => {
    const profile = {
      avoidTraffic: 0.4,
      maxGradient: 12,
      preferCycleways: 0,
  ...
  describe("backward compatibility", () => {
    it("silently drops preferQuietSurfaces and preferLargerRoads from old profiles", () => {
      ...
      // preferSmoothSurfaces defaults to 0 — the rider will need to turn it up
